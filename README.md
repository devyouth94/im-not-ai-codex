...
# Humanize KR for Codex — 한글 AI 티 제거기 v2.3.0

[epoko77-ai/im-not-ai](https://github.com/epoko77-ai/im-not-ai) v2.3.0을 Codex Desktop·CLI용 플러그인으로 옮긴 community adapter입니다. [Squirbie/im-not-ai-codex](https://github.com/Squirbie/im-not-ai-codex)의 포크 계보를 유지하며, 현재 배포본은 [devyouth94/im-not-ai-codex](https://github.com/devyouth94/im-not-ai-codex)에서 관리합니다.

AI(ChatGPT · Claude · Gemini 등)가 쓴 한글 글을 **내용은 한 글자도 건드리지 않고** 문체 · 리듬 · 표현만 자연스러운 한국어로 되돌리는 Codex 스킬입니다.
...
> **Codex Desktop·CLI**를 지원합니다. v2.3.0 설치 검증 환경은 macOS이며 Windows·Linux는 아직 미검증입니다. Claude Code 사용자는 [기능 기준 원본의 안내](https://github.com/epoko77-ai/im-not-ai/tree/v2.3.0#설치-install)를 참고하세요.
...
CLI에서는 다음 명령으로 설치합니다.

```bash
codex plugin add im-not-ai-codex@im-not-ai-codex-marketplace
```

Codex Desktop에서는 앱을 다시 시작한 뒤 **Plugins**에서 `im-not-ai Codex` source의 `Humanize Korean`을 설치합니다. 설치 후에는 새 task 또는 새 CLI 세션을 시작해야 bundled skill이 로드됩니다.

### 업데이트

```bash
codex plugin marketplace upgrade im-not-ai-codex-marketplace
codex plugin remove im-not-ai-codex@im-not-ai-codex-marketplace
codex plugin add im-not-ai-codex@im-not-ai-codex-marketplace
```
...
```bash
codex plugin remove im-not-ai-codex@im-not-ai-codex-marketplace
codex plugin marketplace remove im-not-ai-codex-marketplace
```
...
[prepare_monolith_input.py]  ── 정량 사전 점수 (KatFish·post-editese 지표) + route_hint 산출
    ...
    ↓
[verify_gates.py]            ── 구조 수렴 게이트 (결정적 코드 판정, exit code) — 모든 경로 공통
```
...
Codex 포트의 윤문 실행에는 아래 3개 역할 계약을 사용합니다. 플러그인은 지속적인 custom agent 설정이나 특정 모델을 설치하지 않으며, standard·heavy 경로에서 실제 Codex 서브에이전트로 역할을 호출합니다.

| 런타임 역할 | 경로 | 역할 |
|---------|---|------|
| `humanize-monolith` | 전 경로 공용 | 단일 호출 윤문(탐지·윤문·자체검증 일괄) |
| `humanize-diagnostician` | standard·heavy | 글 전체의 지배 패턴 3~6개 진단, taxonomy ID + 처방 |
| `humanize-finalizer` | heavy·승급 시 | 원문 직접 대조로 의미 보존 15항 + 자연성 판정, 국소 보정 |

기능 기준 원본의 분류학·릴리스 개발 역할과 v2.1에서 은퇴한 역할은 초기 Codex 포트에 번들하지 않습니다.
...
전체 70 서브 패턴(+hold 1건)과 처방: [`ai-tell-taxonomy.md`](plugins/im-not-ai/skills/humanize-korean/references/ai-tell-taxonomy.md) · [`rewriting-playbook.md`](plugins/im-not-ai/skills/humanize-korean/references/rewriting-playbook.md) · 학술 인용 외부 SSOT: [`scholarship.md`](plugins/im-not-ai/skills/humanize-korean/references/scholarship.md) (v2.0 신규)
...
**품질 등급 (윤문 후)**
- **A**: S1 0건, S2 ≤2건, 변경률 10~25%, 자체검증 6항 통과
- **B**: S1 0건, S2 ≤4건, 자체검증 5항 이상 통과
- **C**: S1 1~2건 또는 자체검증 4항 이하 → heavy 재실행 권고
- **D**: S1 3건+ 또는 변경률 50% 초과 → 작업 중단 권고

## 사용법 — 5분이면 따라합니다

### 0. 전제

Codex Desktop 또는 Codex CLI가 필요합니다. CLI 설치 확인:

```bash
codex --version
```

설치 후 새 task 또는 새 CLI 세션에서 사용합니다. 이 릴리스의 실제 설치 검증 환경은 macOS입니다.

### 1. AI가 쓴 한글 글 붙여넣고 부탁하기

평소 말투 그대로 요청하면 됩니다.

```text
humanize-korean으로 이 AI 글 자연스럽게 윤문해줘:

[ChatGPT / Claude / Gemini 초안 여기에 붙여넣기]
```

아래 표현 중 아무거나 쓰면 스킬이 발동합니다.
...
- "번역투 제거"
- "한글 AI 윤문"
- "`$humanize-korean`으로 윤문해줘"

옵션은 요청 끝에 자연어로 적을 수 있습니다: `장르: 칼럼`, `강도: 적극`, `정밀 모드`, `가볍게`. `정밀 모드`는 heavy, `가볍게`는 light 경로를 강제합니다.
...
**standard (2콜 · 보통의 AI 초안 · 2~5분)** — diagnostician 1콜이 지배 패턴을 짚고, monolith 1콜이 겨냥 윤문합니다. 구조 게이트 경고가 남으면 finalizer로 승급할 수 있습니다.

**heavy (3+콜 · 중증 슬롭·초장문·증적 필요 · `정밀 모드`로 강제 가능 · 5~8분)** — diagnostician → monolith(실제 본문 청크가 2개 이상이면 청크 병렬) → finalizer 순으로 처리합니다.

산출물은 `_workspace/{실행날짜-번호}/`에 저장됩니다.

| 파일 | 내용 |
|------|------|
| `01_input.txt` | 원문 그대로 |
| `00_metrics.json` · `01_input_with_metrics.txt` | 정량 사전 점수 + `route_hint` + 점수 블록을 원문 앞에 붙인 결합 입력 |
| `02_diagnosis.md` | standard·heavy의 지배 패턴 3~6개 진단 |
| `08_gate.txt` · `08_gate_final.txt` | 구조 게이트 결과(경로에 따라 최종 파일명 상이) |
| `final_pre_finalize.md` | finalizer 실행 시 보정 전 윤문본 백업 |
| `09_finalize.json` | finalizer의 의미 보존·자연성 판정 |
| `final.md` | 윤문본 + 본문 끝 `<!-- HUMANIZE-SUMMARY -->` 주석 블록 |
...
부분 재실행은 heavy 경로로 전환됩니다.

### 4. 다른 글로 또 돌리고 싶을 때

Codex task에서 새 글을 붙여넣고 똑같이 부탁하면 됩니다. 실행마다 새 `_workspace/{날짜-번호}/` 폴더가 만들어져 이전 결과와 섞이지 않습니다.
...
> 아래 기록은 epoko v2.3.0 README의 제품 이력과 실측 근거를 보존한 것입니다. Claude Code 전용 경로와 당시 파일 배치는 기능 기준 원본을 설명하며, 현재 Codex 설치·실행 방법은 위 섹션을 따릅니다.
...
- **구조 수렴 게이트 (`scripts/verify_gates.py`, LLM 콜 0)** — ① 문자율(폭주 재작성 상한, 기존) ② **진단 목표달성**(진단이 지목한 지표가 실제로 사람 분포로 수렴했는지 z-score로 검증) ③ **대구 전멸 방지**(C-8 "A가 아니라 B"를 다 깨서 필자 목소리를 지우면 실패) ④ golden + **수치 주입 차단**. 문자 change_rate가 못 보던 구조 편집(쉼표·대구 해체)을 결정적으로 검증합니다. 기능 기준 원본은 기존 `verify_change_rate.py`를 하위호환용으로 보존하지만, 이 Codex 포트는 `verify_gates.py`로 통합합니다.
...
- **은퇴 5종** — `ai-tell-detector` · `korean-style-rewriter` · `content-fidelity-auditor` · `naturalness-reviewer`(역할이 진단·finalize 콜로 통합) · `humanize-web-architect`(웹은 별도 코드베이스로 운영, upstream은 설계 문서만 보존하며 이 Codex 포트는 웹 범위를 번들하지 않음)
...
- **학술 인용 양면 보존** — 본진 `taxonomy.md` 패턴마다 `source_anchor` 한 줄(≤25자) + 학자 29명·Caveat 6건 verbatim은 외부 SSOT [`scholarship.md`](plugins/im-not-ai/skills/humanize-korean/references/scholarship.md)에 보존. 룰북 슬림성 유지
...
전체 v1.3.1 변경 이력: [`ai-tell-taxonomy.md` 버전 관리](plugins/im-not-ai/skills/humanize-korean/references/ai-tell-taxonomy.md#버전-관리)
...
회차별 점검 로그는 운영 산출물이라 `_workspace/taxonomy_changelog.md`에 누적되며 (gitignored), 본진 변경은 [`ai-tell-taxonomy.md` 버전 관리](plugins/im-not-ai/skills/humanize-korean/references/ai-tell-taxonomy.md#버전-관리)에 영구 기록됩니다.

전체 v1.3 변경 이력: [`ai-tell-taxonomy.md` 버전 관리](plugins/im-not-ai/skills/humanize-korean/references/ai-tell-taxonomy.md#버전-관리)
...
전체 v1.2 변경 이력: [`ai-tell-taxonomy.md` 버전 관리](plugins/im-not-ai/skills/humanize-korean/references/ai-tell-taxonomy.md#버전-관리)
...
Codex 설치·서브에이전트 실행·경로·패키징 문제는 [이 포트의 Issue](https://github.com/devyouth94/im-not-ai-codex/issues)로 보고해 주세요.

새로운 AI 티 패턴이나 taxonomy 제안은 [기능 기준 원본의 Issue](https://github.com/epoko77-ai/im-not-ai/issues)로 보고할 수 있습니다. 실증 사례 2건 이상(가능하면 서로 다른 모델·장르·작가)이 함께면 upstream 점검 회차에서 본진([`ai-tell-taxonomy.md`](plugins/im-not-ai/skills/humanize-korean/references/ai-tell-taxonomy.md)) 승격을 검토합니다. v1.3에서 운영했던 candidate pool은 핫패스 비용 문제로 v1.5에서 제거됐고, 외부 보고는 Issue 채널로 단순화됐습니다.
...
기능 기준 원본의 기여자 명단과 상세 안내는 [upstream `CONTRIBUTORS.md`](https://github.com/epoko77-ai/im-not-ai/blob/v2.3.0/CONTRIBUTORS.md)를 참고하세요.

## Contributors

v1.2까지의 외부 기여자: **[@simonsez9510](https://github.com/simonsez9510)** (Issue #1, PR #3 — 권한 위계 기반 voice profile 도입), **[@gaebalai](https://github.com/gaebalai)** (Issue #5 LICENSE 지적, Issue #6 슬래시 커맨드 reference + 포크 distribution channel). 전체 명단과 기여 내역: [upstream `CONTRIBUTORS.md`](https://github.com/epoko77-ai/im-not-ai/blob/v2.3.0/CONTRIBUTORS.md).

---

기능 기준 원본은 [Claude Code](https://claude.com/claude-code) + [revfactory/harness](https://github.com/revfactory/harness) 아키텍처 기반입니다. 이 저장소는 해당 v2.3.0 흐름을 Codex 플러그인으로 포팅합니다.
