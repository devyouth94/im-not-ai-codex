...
# metrics.py is at:
#   {plugin_root}/skills/humanize-korean/references/metrics.py
...
def _workspace_root() -> Path:
    return (Path.cwd() / "_workspace").resolve()
...
def _resolve_run_dir(run_dir_arg: str | None, text_arg: str | None) -> Path:
    ...
    if run_dir_arg:
        ...
        rd = raw.resolve() if raw.is_absolute() else (Path.cwd() / raw).resolve()
        try:
            rd.relative_to(workspace)
        except ValueError as exc:
            raise SystemExit(
                f"--run-dir must resolve inside {workspace}; got {rd}"
            ) from exc
        return _prepare_run_dir(rd)
    if text_arg is None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.diagnosis:
        ...
        diag_path = (
            ...
            else (Path.cwd() / diag_path).resolve()
        )
        try:
            diag_path.relative_to(_workspace_root())
        except ValueError as exc:
            raise SystemExit(
                f"--diagnosis must resolve inside {_workspace_root()}; got {diag_path}"
            ) from exc
